...
def direct(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    model = gpflow.models.GPR((x_train, y_m_train), kernel=kernel, mean_function=gpflow.mean_functions.Constant(y_train.mean()))
...
def latentvariable(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    model = gpflow.models.GPR((x_m_train, y_m_train), kernel=kernel, mean_function=gpflow.mean_functions.Constant(y_train.mean()))
...
def difference(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gpflow.models.GPR((x_train, y_m_train), kernel=kernel, mean_function=gpflow.mean_functions.Constant(y_train.mean()-t_train.mean()))
...
def linearprior(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gpflow.models.GPR((x_train, y_m_train), kernel=kernel, mean_function=gpflow.mean_functions.Linear(A=[[0.5],[0.01]],b=y_train.mean()))
...
def fixedprior(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gpflow.models.GPR((x_train, y_m_train), kernel=kernel, mean_function=gpflow.mean_functions.Linear(A=[[1],[0]],b=np.log(1/6)))
