...
def direct(x_train, y_train, t_train, a_train, x_test, y_test, t_test):
    ...
    model = gprhetero.GPRhetero((x_train,
                                 y_train.reshape(-1, 1)),
                                NoiseVar=a_train.reshape(-1, 1),
                                kernel=kernel,
                                mean_function=mean)
...
def latentvariable(x_train, y_train, t_train, a_train, x_test, y_test, t_test):
    ...
    mean = gpflow.mean_functions.Constant(y_train.mean())
    model = gprhetero.GPRhetero((x_m_train, y_train.reshape(-1, 1)),
                                NoiseVar=a_train.reshape(-1, 1),
                                kernel=kernel,
                                mean_function=mean)
...
def difference(x_train, y_train, t_train, a_train, x_test, y_test, t_test):
    ...
    model = gprhetero.GPRhetero((x_train, y_m_train),
                                NoiseVar=a_train.reshape(-1, 1),
                                kernel=kernel,
                                mean_function=mean)
...
def quotient(x_train, y_train, t_train, a_train, x_test, y_test, t_test):
    ...
    # put data together
    y_m_train = y_train.reshape(-1, 1) / t_train.reshape(-1, 1)
    a_m_train = a_train.reshape(-1, 1) / \
        (t_train.reshape(-1, 1) * t_train.reshape(-1, 1))
...
def linearprior(x_train, y_train, t_train, a_train, x_test, y_test, t_test):
    ...
    model = gprhetero.GPRhetero((x_train, y_train.reshape(-1, 1)),
                                NoiseVar=a_train.reshape(-1, 1),
                                kernel=kernel,
                                mean_function=mean)
