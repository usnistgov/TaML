...
for GPR with heteroscedastic noise as implemented in GPFlow.
"""
...
def direct(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    """
    Run the GPR with heteroscedastic noise with no knowledge of theory
    and return the mean squared error. 
    """
   
    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gprhetero.GPRhetero((x_train, y_train.reshape(-1,1)), NoiseVar=a_train.reshape(-1,1), kernel=kernel, mean_function=gpflow.mean_functions.Constant(y_train.mean()))
    ...
    return mean_squared_error(y_test.reshape(-1,1), y_pred)
...
def latentvariable(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    heteroscedastic noise. Return the mean squared error.
    """
    ...
    model = gprhetero.GPRhetero((x_m_train, y_train.reshape(-1,1)), NoiseVar=a_train.reshape(-1,1), kernel=kernel, mean_function=gpflow.mean_functions.Constant(y_train.mean()))
...
def difference(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    """
    Use GPR with heteroscedastic noise to predict the difference between the 
    ...
    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gprhetero.GPRhetero((x_train, y_m_train), NoiseVar=a_train.reshape(-1,1), kernel=kernel, mean_function=gpflow.mean_functions.Constant(y_train.mean()-t_train.mean()))
...
def quotient(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    # put data together
    y_m_train = y_train.reshape(-1,1)/t_train.reshape(-1,1)
    a_m_train = a_train.reshape(-1,1)/(t_train.reshape(-1,1)*t_train.reshape(-1,1))

    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gprhetero.GPRhetero((x_train, y_m_train), NoiseVar=a_train.reshape(-1,1), kernel=kernel, mean_function=gpflow.mean_functions.Constant(y_train.mean()))
...
def linearprior(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    """
    Use GPR with heteroscedastic noise for predictions. The prior is assumed to
    ...
    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gprhetero.GPRhetero((x_train, y_train.reshape(-1,1)), NoiseVar=a_train.reshape(-1,1), kernel=kernel, mean_function=gpflow.mean_functions.Linear(A=[[0.5],[0.01]],b=y_train.mean()))
...
def fixedprior(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    ...
    # model construction
    kernel = gpflow.kernels.SquaredExponential(lengthscales=[3,0.1])
    model = gprhetero.GPRhetero((x_train, y_train.reshape(-1,1)), NoiseVar=a_train.reshape(-1,1), kernel=kernel, mean_function=gpflow.mean_functions.Linear(A=[[1],[0]],b=np.log(1/6)))
...
def parameterization(x_train,y_train,t_train,a_train,x_test,y_test,t_test):
    """
    Instead of modeling the data using a single GPR with heteroscedastic noise,
    ...
    set_trainable(model.kernel.kernels[0].kernels[0].variance, False) # fix one of the two prefactors

    # optimize
    model = optimizeGPR(model,15000) # need more iterations as more parameters to minimize over
